/* Devizio — application devis & factures pour paysagistes.
...
(function () {
  ...
  /* ---------- Store ---------- */
  var db = load();
  var LOGGED_IN = !!localStorage.getItem("devizio_token");
  try { if (!db.settings.trade) { var _t = localStorage.getItem("devizio_trade"); if (_t) db.settings.trade = _t; } } catch (e) {}
  function load() {
    try {
  ...
  var GENERIC_PRESETS = ["Prestation", "Fourniture", "Main d'œuvre (heure)", "Déplacement", "Forfait"];
  function currentPresets(){ var t = db.settings.trade; return (window.TRADES && window.TRADES[t] && window.TRADES[t].presets) || GENERIC_PRESETS; }
  ...
  function renderDashboard(v) {
    ...
    v.innerHTML =
      '<div class="page-head"><div><h1>Tableau de bord</h1><div class="sub">Bonjour ' + (esc(db.settings.company.nom) || "à vous") + ' 👋</div></div>' +
  ...
  function renderEditor(v, type, editId) {
    ...
    v.innerHTML =
      ...
      '<div class="panel"><div class="panel-body">' +
        ...
        '</div>' +
        '<datalist id="presets">' + currentPresets().map(function (p) { return '<option value="' + esc(p) + '">'; }).join("") + '</datalist>' +
  ...
  function renderDoc(v, id) {
    ...
    v.innerHTML =
      ...
      '<div style="padding:24px;background:var(--bg-soft)"><div class="doc-a4" id="doc-print">' +
        ...
        '</div>' +
        ((db.settings.isPro || LOGGED_IN) ? '' : '<div class="doc-watermark">Réalisé avec Devizio — devizio.fr</div>') +
      '</div></div>';
  ...
  function renderSettings(v) {
    ...
    v.innerHTML =
      ...
      '<div class="panel"><div class="panel-head"><h2>Mon entreprise</h2></div><div class="panel-body">' +
        '<div class="field"><label>Métier</label><select id="s-trade">' + tradeOptions(db.settings.trade) + '</select><div class="hint">Détermine les prestations pré-remplies dans vos devis.</div></div>' +
      ...
      '<div class="panel"><div class="panel-head"><h2>Abonnement</h2></div><div class="panel-body">' +
        ...
        (db.settings.isPro
          ...
          : '<p style="color:var(--text-soft)">Le plan gratuit est limité à ' + FREE_LIMIT + ' documents et ajoute une mention Devizio sur les PDF.</p><button class="btn btn-primary" id="upgrade">✨ Passer à Pro</button>') +
      '</div></div>';
    ...
    $("#s-save").onclick = function () {
      ...
      db.settings.trade = $("#s-trade").value || null; try { if (db.settings.trade) localStorage.setItem("devizio_trade", db.settings.trade); } catch (e) {}
  ...
  function upgradeModal() {
    openModal(
      ...
      '<ul style="list-style:none;padding:0;margin:14px 0 20px;color:var(--text-soft)">' +
        ...
        '<li>✓ PDF <strong>sans mention</strong> Devizio</li>' +
  ...
  /* ---------- Métier ---------- */
  function tradeOptions(sel) {
    var order = window.TRADE_ORDER || [];
    var opts = '<option value="">— choisir un métier —</option>';
    order.forEach(function (k) { var t = window.TRADES[k]; opts += '<option value="' + k + '"' + (k === sel ? " selected" : "") + '>' + t.emoji + ' ' + esc(t.name) + '</option>'; });
    return opts;
  }
  function setTrade(k) { db.settings.trade = k; try { localStorage.setItem("devizio_trade", k); } catch (e) {} save(); closeModal(); updateProTag(); router(); }
  function maybeChooseTrade() {
    if (!LOGGED_IN) return;
    if (db.settings.trade && window.TRADES && window.TRADES[db.settings.trade]) return;
    var order = window.TRADE_ORDER || [];
    if (!order.length) return;
    var grid = order.map(function (k) { var t = window.TRADES[k]; return '<button class="trade-pick" data-t="' + k + '"><span class="e">' + t.emoji + '</span><span>' + esc(t.name) + '</span></button>'; }).join("");
    openModal('<h3>Quel est votre métier ?</h3><p style="color:var(--text-soft);margin:0 0 16px">On pré-remplit vos prestations en fonction.</p><div class="trade-grid">' + grid + '</div>', function () {
      $$(".trade-pick").forEach(function (b) { b.onclick = function () { setTrade(b.dataset.t); }; });
    });
  }
